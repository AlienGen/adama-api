...
public abstract class TokenProviderAbstract<D extends DeleteEntityAbstract, A extends AdamaUser<D>> {
	...
	public String createAccessToken(Authentication authentication, ZonedDateTime validity) {
		...
		return user
				...
				.map(u -> Jwts.builder().setSubject(authentication.getName()).claim(AUTHORITIES_KEY, authorities).claim(TENANT_ID, u.getTenant().getId())
						.signWith(SignatureAlgorithm.HS512, adamaProperties.getSecurity().getAuthentication().getJwt().getSecret()).setExpiration(expiredDate).compact())
				.orElse(Jwts.builder().setSubject(authentication.getName()).claim(AUTHORITIES_KEY, authorities)
						.signWith(SignatureAlgorithm.HS512, adamaProperties.getSecurity().getAuthentication().getJwt().getSecret()).setExpiration(expiredDate).compact());
	}
	...
	public String createRefreshToken(Authentication authentication) {
		...
		return Jwts.builder().setSubject(authentication.getName()).claim(AUTHORITIES_KEY, authorities)
				.signWith(SignatureAlgorithm.HS512, adamaProperties.getSecurity().getAuthentication().getJwt().getSecret()).setExpiration(expiredDate)
				.setIssuedAt(Date.from(ZonedDateTime.now().toInstant())).compact();
	}
	...
	public Authentication getAuthentication(String token) {
		...
		Collection<? extends GrantedAuthority> authorities = Arrays.asList(claims.get(AUTHORITIES_KEY).toString().split(",")).stream().map(authority -> new SimpleGrantedAuthority(authority))
	...
	public boolean validateToken(String authToken) throws JwtException {
		// if any problem with the token an exception will be thrown: throws
